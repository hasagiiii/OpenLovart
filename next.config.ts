...
// to the local Go backend so the Next.js app can call them via same-origin
// fetch (cookies + CSRF stay simple). In production we expect the frontend
// and backend to be served from the same hostname (or the deployer can
// re-create equivalent rewrites at the edge).
